...
summarize_bills <- function(df) {
  ...
  out <- df |> 
    summarise(count = n(), mean_bill_length = mean(bill_length_mm, na.rm = T), max_bill_length = max(bill_length_mm, na.rm = T))
  return(out)
}
...
# load packages
library(tidyverse)
library(legislatoR) # Changed to lowercase

# get political data on German legislators
political_df <- # Changed ger to deu
  left_join(x = filter(get_political(legislature = "deu"), as.numeric(session) == 18), # Changed from string to variable
...
# sessions served
sessions_served_df <- 
  get_political(legislature = "deu") %>% # Added >
  group_by(pageid) |> 
...
legislator_df_table <- dplyr::select(legislator_df, rank, name, traffic_mean, traffic_max) # Added data source
