...
**Exercise 4**: How would you find the maximum value for number of conflict events using the `$` operator?
```{r, echo = F}

```

**Exercise 5**: Print the country that corresponds to the maximum value for conflict events using the `$` operator and indexing!
```{r, echo = F}
...
## Tidy Data `r emo::ji("card_file_box")`
