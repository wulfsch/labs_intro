...
- Second, we will discuss some best practices in R. 
- Third, we will dive into the Tidyverse, a collection of R packages that are designed to work together to make data science easier and more intuitive.
...
As much as possible I recommend you avoid using ChatGPT or any other AI tools for coding help in these labs. The point of these labs is to learn how to code yourself, so make sure you always try each task yourself first and that you understand each line of code.
...
## Adding observations
First, let's add another observation to the data. Suppose we wanted to add a hypothetical observation for a new Adelie penguin. We can use the rbind() function to do so. rbind() stands for "row bind." Save the output in a new data frame!
...
## Adding variables
We can also create new variables that use information from the existing data. For example, we might want to create a ratio of bill length to bill depth, or convert body mass from grams to kilograms. By using the $ operator, we can directly assign the new variable to the data frame.
```{r}
penguins$bill_ratio <- penguins$culmen_length_mm / penguins$bill_depth_mm
penguins$body_mass_kg <- penguins$body_mass_g / 1000

head(penguins, 3) # prints out the first 3 rows of the data frame
```

## Subsetting data 
Suppose we want to figure out which Adelie penguin has the longest flipper. We can figure this out by first subsetting our dataset to only include Adelie penguins, then looking up the maximum value for flipper_length_mm.
```{r}
adelie_penguins <- penguins[penguins$species == "Adelie",]
max(adelie_penguins$flipper_length_mm, na.rm = TRUE)

# Find which Adelie penguin has the maximum flipper length
max_flipper_length <- max(adelie_penguins$flipper_length_mm, na.rm = TRUE)
adelie_penguins[adelie_penguins$flipper_length_mm == max_flipper_length, ]
```

:::alert-info
**Exercise 5**

Create a subset containing only female penguins from Biscoe island. How many observations are in this subset?
:::
...
## Tidy Data `r emo::ji("file_cabinet")`
...
For those of you who have yet to create a Github account, do so [now](https://github.com/). We will be using Github extensively in this course so make sure you get familiar with version control and project management with Git and Github. Eveyone has their own preferences, but to get started either GitHub Desktop or the RStudio Git interface are good options. While we won't have a lab session on the topic, I have included materials from a previous version of this course under session-00 which you should go through if you still feel unsure about how to use Git and Github.
