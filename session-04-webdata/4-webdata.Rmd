...
```{r}
# Your code here
unlist(str_extract_all(stringr::words, ".*ing|.*ise"))
```
:::
...
| Specification | Meaning | Shorthand version |
  ...
  | `[:alnum:]`  | Digits and alphabetic characters | \\w |
...
```{r}
# Your code here
unlist(str_extract_all(stringr::words, ".*[^e]ed$"))
```
:::
...
```{r}
# Your code here
sum(str_detect(stringr::words, "^.{2}y"))
```
:::
:::alert-info
...
```{r}
# Your code here
str_extract_all(example.obj, "\\b[:alpha:]{1,5}\\b")
```
:::
...
str_extract_all(emails, ".+@students\\.hertie-school\\.org")
```
...
str_extract_all(ex_string, "[:digit:]+.{0,2}[:digit:]+.?[:digit:]+[:alpha:]+.{0,2}[:alpha:]+.{0,2}[:alpha:]+")
```
