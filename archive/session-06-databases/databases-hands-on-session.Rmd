...
# Working with the Comparative Legislators Database (Scotland edition)

In the following, we will work with the [Comparative Legislators Database (CLD)](https://complegdatabase.com/). The CLD is a collection of datasets on politicians from around the world. The database contains information for over 67,000 contemporary and historical legislators from 16 countries. It unites collaborative micro-data collection efforts and brings these together through the integration of data from Wikipedia, Wikidata, and other sources. Here, we're working with data on Scottish MPs.

```{r, eval = TRUE}
con <- dbConnect(RSQLite::SQLite(), dbname = "sco.sqlite")

# summarize content of the database
dbListTables(con)

# identify fields of a specific table
dbListFields(con, "history")

# preview a specific table
dbGetQuery(con, "SELECT * FROM history LIMIT 5")

# identify keys to link tables "history" and "core"
dbGetQuery(con, "PRAGMA table_info('history')")
dbGetQuery(con, "PRAGMA table_info('core')")
```
