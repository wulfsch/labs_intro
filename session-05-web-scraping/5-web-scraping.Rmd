...
# YOUR ANSWER HERE
parsed_url |> 
  rvest::html_elements(xpath = '/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/p[7]') |> 
  rvest::html_text() |> 
  str_trim()
```
...
# Function for exercise 2b

split_artists <- function(str) {
  parts <- strsplit(str, ", | and | with")[[1]] |> 
    str_remove_all("I") |> 
    str_replace("Tones", "Tones and I") |> 
    trimws()
  return(parts)
}
spotify_artists <- spotify_table |> 
  mutate(unique_artists = map(artist_s, split_artists)) |> 
  select(unique_artists) |> 
  unnest_wider(col = unique_artists, names_sep = "_")

spotify_artists |> 
  pivot_longer(cols = everything(), values_to = "uniqueness") |> 
  group_by(uniqueness) |> 
  summarise(count = n()) |> 
  arrange(desc(count))
```
...
```{r}
# YOUR ANSWER HERE
new_spotify <- spotify_table |> 
  separate_rows(artist_s, sep = ", | with | and") |> 
  filter(str_trim(artist_s) != "I") |> 
  mutate(artist_s = ifelse(artist_s == "Tones", "Tones and I", artist_s)) |> 
  group_by(artist_s) |> 
  summarise(total_streams = sum(streams_billions, na.rm = T), .groups = "drop") |> 
...
# Remember adding them to .gitignore!!!
